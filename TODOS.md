...
- [ ] Create `supabase/migrations/20260414000000_player_answers.sql` with the `player_answers` table (id, player_id FK, question_id FK, answer_id FK, created_at) and a `UNIQUE (player_id, question_id)` constraint
- [ ] Run `nix run nixpkgs#supabase-cli -- db push` to apply the migration to the remote DB
...
- [ ] Add `playerCount` state initialised to `0`
- [ ] Add `players` state initialised to `[]`
- [ ] In the existing session subscription `useEffect`, after confirming `sessionId` exists, subscribe to a channel named `players-${sessionId}` filtered to `players` table with `session_id=eq.${sessionId}`
- [ ] In the subscription's `on('INSERT')` handler, increment `playerCount` and append the new player to `players`
- [ ] Return `() => supabase.removeChannel(channel)` from the `useEffect`
- [ ] In the session-fetch `then()` block, also load existing players for the session: `select('*').eq('session_id', sessionId).order('joined_at')` and set both states
- [ ] Update the `waiting` block UI: change "Waiting for players…" text to "X player(s) joined" and update the Start Game button label to "Start game (X players)"
...
- [ ] Add `const [answerCount, setAnswerCount] = useState(0)` state
- [ ] Add `const [questionOpen, setQuestionOpen] = useState(true)` state (initialise from session subscription payload — default `true` for backwards compatibility with existing sessions that have no `question_open` column yet; after migration the DB default handles this)
- [ ] Add a realtime subscription on `player_answers` table filtered to the current question_id (`question_id=eq.${questions[currentQuestionIndex]?.id}`). Increment `answerCount` on each INSERT. Use channel name `answers-${sessionId}`.
- [ ] In the `on('UPDATE')` handler on sessions, destructure `question_open` from `payload.new` and update `setQuestionOpen`
- [ ] When `currentQuestionIndex` changes, reset `answerCount` to `0`
- [ ] Update `nextQuestion()`: include `question_open: true` in the `.update()` call so opening the next question also resets the open flag
- [ ] Add a new `closeQuestion()` function: `.update({ question_open: false })` on sessions, guarded by `if (!questionOpen) return`
- [ ] In the `active` state UI: add a "Close question" button above the Next/End buttons. Disabled when `!questionOpen`. Show "X / Y answered" text below the question counter, where Y is `playerCount`
- [ ] When `!questionOpen` (after the host has closed), change the "answered" text to "Results shown" (feedback is now visible to players)
...
- [ ] Add state: `const [answerSubmitted, setAnswerSubmitted] = useState(false)` and `const [alreadyAnswered, setAlreadyAnswered] = useState(false)`
- [ ] Rename `selectedAnswerId` → `submittedAnswerId` for semantic clarity (or keep the name, your call — this is a cosmetic refactor)
- [ ] In `handleAnswer`, rename the function to `submitAnswer(answer)` and guard with `if (answerSubmitted || alreadyAnswered) return`
- [ ] Inside `submitAnswer`: set `submittedAnswerId`, then INSERT into `player_answers` with `{ player_id, question_id: questions[currentQuestionIndex].id, answer_id: answer.id }`
- [ ] On INSERT success: set `answerSubmitted(true)`, show a subtle "Answer submitted" confirmation below the answer buttons (no correctness yet)
- [ ] On INSERT error (constraint violation code `23505`): set `alreadyAnswered(true)` and show "You already answered this question"
- [ ] Disable all answer buttons when `answerSubmitted || alreadyAnswered` (in addition to the existing guard)
...
- [ ] Add state: `const [feedbackShown, setFeedbackShown] = useState(false)`, `const [isCorrect, setIsCorrect] = useState(null)`, `const [pointsEarned, setPointsEarned] = useState(0)`, `const [leaderboard, setLeaderboard] = useState([])`
- [ ] Add a `useRef`: `const wasActiveRef = useRef(false)`. Set `wasActiveRef.current = true` after questions load in the session subscription callback.
- [ ] In the session subscription's `on('UPDATE')` handler, detect the `question_open` transition: `if (wasActiveRef.current && payload.new.question_open === false) { /* reveal feedback */ }`
- [ ] In the feedback reveal block: set `feedbackShown(true)`, reset `answerSubmitted(false)`, `submittedAnswerId(null)`, `alreadyAnswered(false)`. Fetch `player_answers` row for `{ player_id, question_id: questions[payload.old.current_question_index].id }`, join with `answers` to get `is_correct`, derive `pointsEarned = isCorrect ? questions[oldIndex].points : 0`. Fetch `players` for `session_id` ordered by `score desc`, `nickname asc` → set `leaderboard`.
- [ ] Update `answerClassName`: when `feedbackShown` is `true`, show the player's submitted answer in green (if correct) or red (if wrong) with a ring. Show the correct answer (the one with `is_correct === true`) in emerald green with a ring even if it wasn't the player's answer. All other answers dim to 40% opacity.
- [ ] In the feedback reveal block, also show a result banner above the answers: "Correct! +X points" in green or "Wrong" in red, using the `isCorrect` and `pointsEarned` state.
- [ ] When `currentQuestionIndex` changes to a new value (session UPDATE with new index), reset: `setFeedbackShown(false)`, `setAnswerSubmitted(false)`, `setSubmittedAnswerId(null)`, `setAlreadyAnswered(false)`, `setIsCorrect(null)`, `setPointsEarned(0)`
- [ ] Add a `leaderboardView` section that replaces the entire inner content (question + answers) when `feedbackShown` is `true`. Render a ranked list: rank number, nickname (highlight the player's own row with a subtle background), and score. Show the result banner above it. "Waiting for next question…" text below the leaderboard if `questionOpen` is `false` and we're still in `active` state.
- [ ] The existing `useEffect` that resets `submittedAnswerId` on `currentQuestionIndex` change should also reset `feedbackShown` and `alreadyAnswered`
...
- [ ] Change `.eq('state', 'waiting')` to `.in('state', ['waiting', 'active'])` in the session lookup
